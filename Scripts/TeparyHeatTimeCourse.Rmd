...
ggsave("../Results/Tepary_var_F_scatter.svg", width = 6, height = 5.5)
ggsave("../Results/Tepary_var_F_scatter.png", width = 6, height = 5.5)
```
...
I think modules 9, 3, and 7 look interesting. 
```{r}
...
Let's say you are interested in a few of those kinases you found. 
We can also graph them. 
```{r}
Exp_table_long %>% 
...
```{r}
write_excel_csv(my_network_modules, "../Results/Tepary_network_modules.csv") 
write_excel_csv(modules_mean_z_reordered, "../Results/Tepary_modules_means_z_reordered.csv")
write_excel_csv(Exp_table_long_averaged_z_high_var_or_high_F_modules, 
                "../Results/Tepary_Exp_table_long_averaage_z_high_var_or_high_F_modules.csv")

write_excel_csv(
  Exp_table_long_averaged_z_high_var_or_high_F_modules %>% 
  mutate(tag = paste(time_point, treatment, sep = ";")) %>% 
  select(tag, mean.logFPKM, LocusName) %>% 
  pivot_wider(names_from = tag, values_from = mean.logFPKM),
  "../Results/Tepary_expression_matrix_for_WGCNA.csv"
)
