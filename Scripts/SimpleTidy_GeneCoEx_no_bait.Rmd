...
**This a version of the workflow that does not use any bait genes.** 
...
This markdown file is a version that does not use any bait genes. 
...
ggsave("../Results/gene_var_distribution_no_bait.svg", height = 3.5, width = 3.5)
ggsave("../Results/gene_var_distribution_no_bait.png", height = 3.5, width = 3.5)
```
...
```
We are now down to 1567354 edges. Still **A LOT**. 
...
dim(node_table)
```
We have 4978 genes in this network, along with 1567354 edges.
...
* module 5, a module goes up later in development. 
...
However, this is a workflow that does not use bait genes, so we won't be able to do this analysis. 
...
A common "ultimate" goal for gene co-expression analyses is to find new candidate genes that have an expression pattern of interest. 
Let's say we are interested in ripening genes, which are captured by Module 5. 
This is easy to see if you look at the heatmap that we just made. 
...
ggsave("../Results/Candidate_genes_TPM_no_bait.svg", height = 6, width = 8, bg = "white")
ggsave("../Results/Candidate_genes_TPM_no_bait.png", height = 6, width = 8, bg = "white")
```
As expected, they all go up as the fruit ripens. 

## Write out results
Finally, I want to write out genes in Module 5 as a table onto the hard drive. 
That's easy. 

```{r}
Module5_genes <- my_network_modules %>% 
  filter(module == 5)

head(Module5_genes)
write_excel_csv(Module5_genes, "../Results/Ripening_genes.csv", col_names = T)
